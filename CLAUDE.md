...
- Current version: **0.3.0** (HDRP film look, textured house, high poly furniture, physics).
...
- **Furniture** (`Assets/Editor/FurnitureImport.cs` + the builder's `Layout` table): see `docs/PIPELINE.md`. Blender slot names map to textured materials in `FurnitureImport.Looks`. Pieces so far (high poly, one child per part): `sofa`, `marbletable`, `geomrug` (with tassels), `cushion` x2, all in the living room.
